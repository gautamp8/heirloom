...
| D | **Per-nominee passphrase hashes** — the welcome flow currently accepts a hardcoded dev passphrase (`"the long road home"`) | shipped as dev shortcut | Real per-nominee argon2id passphrase hashes need a column on `nominees` and the creator-side flow that generates the printed-letter passphrase. The executor passphrase path already does this — same shape applies to the recipient nominee | ~1 hr; column migration + hash on nominee assignment + verify in `/api/auth/nominee-passphrase` |
| D4 | **Creator-side nominees list UI** — manage nominees, set release conditions, generate per-nominee letters | not started | Backend (`POST /api/nominee`) ready and the bootstrap covers the demo. The admin UI is needed for self-serve creator workflow but not for the visible product narrative | ~1.5 hr; new `/nominees` route + add-nominee modal + release-condition picker |
| D | **Executor rate limit via Redis** | shipped with in-memory map | Resets on dev-server restart; would lose state across instances. A real production deployment needs Redis (or DB-backed counter) | ~30 min once Redis is provisioned |
